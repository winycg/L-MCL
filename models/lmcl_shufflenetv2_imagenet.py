...
class Auxiliary_Classifier(nn.Module):
    def __init__(self, net_size, num_classes=100):
        ...
        self.in_channels = 24
        self.block_extractor1 = nn.Sequential(*[self._make_layer(out_channels[0], 1),
                                                self._make_layer(out_channels[1], 1),
                                                self._make_layer(out_channels[2], 1),
        ...
        self.block_extractor2 = nn.Sequential(*[self._make_layer(out_channels[1], num_blocks[1]),
        ...
        self.block_extractor3 = nn.Sequential(*[self._make_layer(out_channels[2], num_blocks[2]),
                                                nn.Conv2d(out_channels[2], out_channels[3],
                                                kernel_size=1, stride=1, padding=0, bias=False),
                                                nn.BatchNorm2d(out_channels[3]),
                                                nn.ReLU(inplace=True)])
...
class ShuffleNetV2_n(nn.Module):
    def __init__(self, net_size, num_classes=100, number_net=2):
        ...
        for i in range(number_net):
            self.module_list.append(ShuffleNetV2_Auxiliary(net_size, num_classes=num_classes))
...
def lmcl_ShuffleNetV2_05x_imagenet(num_classes, number_net):
    return ShuffleNetV2_n(net_size=0.5, num_classes=num_classes, number_net=number_net)


def lmcl_ShuffleNetV2_1x_imagenet(num_classes, number_net):
    return ShuffleNetV2_n(net_size=1, num_classes=num_classes, number_net=number_net)


if __name__ == '__main__':
    net = ShuffleNetV2_n(1, num_classes=1000, number_net=2)
