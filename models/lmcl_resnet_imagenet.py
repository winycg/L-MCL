...
class multi_ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, number_net=2):
        ...
        for i in range(number_net):
            self.module_list.append(ResNet_Auxiliary(block=block, layers=layers, num_classes=num_classes))
...
def lmcl_resnet18_imagenet(**kwargs):
    return multi_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)

def lmcl_resnet34_imagenet(**kwargs):
    return multi_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs)

def lmcl_resnet50_imagenet(**kwargs):
    return multi_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)


if __name__ == '__main__':
    net = lmcl_resnet50_imagenet(num_classes=1000)
